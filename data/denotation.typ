...
#(thesis.denotation)(
  (
    ...
    (
      [HPCE],
      [高效毛细管电泳色谱（High Performance Capillary lectrophoresis）],
    ),
    ([LC-MS], [液相色谱-质谱联用（Liquid Chromatography-Mass Spectrum）]),
    ([TIC], [总离子浓度（Total Ion Content）]),
    ([#emph[ab initio]], [基于第一原理的量子化学计算方法，常称从头算法]),
    ...
    ($Delta G^(dagger.double)$, [活化自由能（Activation Free Energy）]),
    ($kappa$, [传输系数（Transmission Coefficient）]),
    ([IRC], [内禀反应坐标（Intrinsic Reaction Coordinates）]),
    ($nu_i$, [虚频（Imaginary Frequency）]),
    (
      [ONIOM],
      [分层算法（Our Own N-layered Integrated Molecular Orbital and Molecular
        Mechanics）],
    ),
